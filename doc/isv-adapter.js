...
KISSY.add(function (S, Gallery) {
    ...
    function init(frameGroup) {
         ...
         */
        function SafeZhaoest(el, config) {
            this.inner = new Zhaoest(el, config);//内部实际封装了真正的组件构造函数
        }

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法，on和destory只是举例，哪些需要暴露的方法就写哪些就行了
        SafeZhaoest.prototype.on = function (type,fnc) {
            //this.inner.on(type,fnc);
        };

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法
        SafeZhaoest.prototype.destory = function () {
            //this.inner.destory();
        };
        ...
        frameGroup.markCtor(SafeZhaoest);

        frameGroup.grantMethod(SafeZhaoest, "on");
        frameGroup.grantMethod(SafeZhaoest, "destory");
        ...
        return function (context) {
            ...
            //最终需要返回给
            return {
                //这里的 zhaoest 会自动放到KISSY.Zhaoest:
            Zhaoest: frameGroup.markFunction(function () {
                    return new SafeZhaoest(arguments[0], cajaAFTB.untame(arguments[1]));
                }),
                kissy:true
            }
        }
